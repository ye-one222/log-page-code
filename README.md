...
`src/content/`는 별도 private 리포([mypage-content](https://github.com/yewon-akkang/mypage-content))이고,
이 코드 리포는 콘텐츠를 추적하지 않는다. 덕분에 코드 리포를 public으로 공개해도
비공개 메모(`publish: false`)는 노출되지 않는다.

이 코드 리포를 새로 클론했다면 콘텐츠를 받아온다:

```bash
git clone https://github.com/yewon-akkang/mypage-content src/content
```

## Obsidian으로 글쓰기

`src/content/`를 Obsidian vault로 연다 (Obsidian → Open folder as vault).
커뮤니티 플러그인은 vault마다 따로 설치되므로, Git 플러그인도 이 vault에서 설치해야 한다.
